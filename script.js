...
let paymentMethodsCache = { qris: true, shopee: true }; // default aktif semua sebelum data server datang
...
// Tanggal terakhir "Refresh Absen" dipakai (format YYYY-MM-DD) — diisi via Firestore listener
// (config/refreshAbsenLog), dipakai buat batasi tombol Refresh Absen maksimal 1x per hari,
// konsisten di semua HP Admin (bukan cuma di HP yang mengetuk tombolnya).
let refreshAbsenLastDate = null;
...
function init() {
    ...
    // Cek pergantian hari tiap menit (bukan cuma saat ada perubahan data dari server) — supaya
    // begitu lewat jam 00:00 sambil app tetap terbuka: (1) kode QR/manual otomatis ikut berganti
    // ke kode hari baru, (2) shift kemarin yang lupa di-absen-keluar otomatis ditutup, dan
    // (3) gerbang wajib absen masuk langsung berlaku lagi untuk hari yang baru.
    setInterval(() => {
        try {
    ...
    // Kalau tab/app sempat di-background (misal pas izin kamera buat scan QR absen muncul, atau
    // kasir pindah app sebentar), browser bisa jeda timer JS. Begitu balik aktif lagi, langsung
    // cek ulang gerbang absen — jangan sampai nyangkut kekunci nunggu detik berikutnya.
...
function initFirestoreSync() {
    ...
    onSnapshot(doc(db, 'config', 'employees'), (snap) => {
        ...
        try {
            ...
            if (adminPanelOpen) renderStockManageList(); // kolom stock per kasir ikut nambah/hilang kalau daftar karyawan berubah
    ...
    // --- Katalog Per Karyawan (produk & stock kustom milik masing-masing kasir, diatur Admin) ---
    onSnapshot(doc(db, 'config', 'employeeCatalog'), (snap) => {
        ...
        if (adminPanelOpen) {
            ...
            renderStockManageList(); // kolom stock per kasir di "Kelola Stock" ikut sinkron
            renderTransferStockTable(); // tabel & pilihan kasir di "Transfer Stock Produk" ikut sinkron
        }
    ...
    // Koleksi lain (schedule, employeeCatalog, dst) TETAP di sini karena memang dipakai
    // langsung di alur kasir (gerbang wajib absen, stock per kasir, katalog per kasir).
    ...
    // --- Metode Pembayaran (Cash selalu aktif; QRIS & Online/Shopee bisa dimatikan Admin) ---
    onSnapshot(doc(db, 'config', 'paymentMethods'), (snap) => {
        trackPendingWrites(snap);
        const data = snap.exists() ? snap.data() : {};
        // Default AKTIF kalau belum pernah diatur sama sekali (fitur ini nambah kontrol, bukan
        // ujug-ujug mematikan metode yang sebelumnya sudah jalan).
        paymentMethodsCache = {
    ...
    // Catatan: dulu ada listener terpisah buat 'config/attendanceQrToken' di sini, tapi kode QR
    // sekarang dihitung otomatis dari tanggal (lihat computeDailyAttendanceToken) jadi tidak
    // perlu tersimpan/disinkronkan lewat Firestore lagi.

    // --- Catatan "Refresh Absen" terakhir (buat batas 1x per hari, sama di semua HP Admin) ---
    onSnapshot(doc(db, 'config', 'refreshAbsenLog'), (snap) => {
        trackPendingWrites(snap);
        refreshAbsenLastDate = (snap.exists() && snap.data().lastDate) ? snap.data().lastDate : null;
        renderRefreshAbsenButton();
    }, (err) => console.error('Sync log refresh absen gagal:', err));
    ...
    onSnapshot(collection(db, 'attendance'), (snap) => {
        ...
        try {
            renderRiwayatAbsen();
            loadPerbaikanAbsenRecord(); // status Perbaikan Absensi ikut update real-time
            checkMandatoryMasukGate();
            ...
            tryShowPinLock();
            autoCloseForgottenShifts(); // tutup otomatis shift kemarin yang lupa absen keluar
...
// Dipanggil saat kartu produk yang REDUP (stock/kuota 0) di-tap/klik oleh kasir — kasih tahu
// jelas kenapa produk itu tidak bisa dimasukkan ke keranjang/metode bayar, daripada diam saja.
...
function renderAdminTools() {
    ...
    renderPaymentMethodsAdmin(); // status toggle QRIS/Online sesuai data terkini tiap panel admin dibuka
...
// Stock sekarang MURNI per kasir (tidak ada lagi stock global gabungan). Tiap produk punya
// satu kolom input per karyawan terdaftar — isi angkanya = jatah stock milik kasir itu SENDIRI
// untuk produk ini (terpisah total dari kasir lain). Kosongkan = kasir itu dianggap boleh jual
// produk ini tanpa batas. Datanya disimpan di struktur yang sama dengan employeeCatalogCache
// ("Katalog Per Karyawan"), jadi otomatis nyambung ke halaman kasir & fitur Transfer Stock.
function renderStockManageList() {
    ...
    list.innerHTML = products.map(p => `
        <div class="bg-slate-50 border border-slate-100 rounded-xl p-3 space-y-2.5">
            <div class="min-w-0">
            ...
            <div class="grid grid-cols-2 gap-2">
                ${employeesCache.map(emp => {
                    ...
                    return `
                    <div class="min-w-0">
                        <label class="block text-[9px] font-bold text-slate-400 uppercase mb-1 truncate">Stock ${emp.name}</label>
                        <input
                            type="number"
                            min="0"
                            value="${qty ?? ''}"
                            placeholder="∞"
                            data-stock-emp="${emp.id}"
                            data-stock-product="${p.id}"
                            class="w-full p-2 text-center border border-slate-200 rounded-lg outline-none text-xs font-bold"
                        >
                    </div>`;
                }).join('')}
            </div>
...
// Ambil kuota/stock khusus milik satu kasir buat satu produk (null = belum diatur atau kosong
// = dianggap tidak terbatas). Sumber datanya SAMA PERSIS dengan employeeCatalogCache yang
// dipakai "Katalog Per Karyawan" — cuma diedit lewat UI Kelola Stock di sini.
...
// Simpan SEMUA perubahan stock sekaligus lewat tombol "Simpan Stock" di paling bawah menu
// "Kelola Stock" — BUKAN otomatis tersimpan tiap kali angka diketik/onchange lagi.
// Alasan diganti dari onchange-langsung-simpan ke tombol Simpan: sebelumnya tiap input
// nge-trigger setDoc sendiri-sendiri berdasarkan employeeCatalogCache saat itu; kalau Admin
// mengetik cepat di lebih dari satu kolom (produk berbeda, kasir yang sama) SEBELUM balasan
// server & listener snapshot dari edit pertama sempat kembali, edit kedua ikut membangun
// array dari cache yang masih lama (belum ke-update) lalu menimpa balik seluruh array —
// hasilnya edit pertama seperti "hilang"/tidak update. Dengan tombol Simpan, semua input di
// layar dibaca sekali jalan lalu dikirim dalam satu batch per kasir, jadi tidak ada lagi
// balapan antar-edit yang saling menimpa.
function saveStockManageChanges() {
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
    if (employeesCache.length === 0) return;

    // Mulai dari data terakhir yang sudah tersimpan di server (employeeCatalogCache), supaya
    // status "aktif"/checkbox yang diatur lewat "Katalog Per Karyawan" tidak ikut ketiban rusak.
    const updatesByEmp = {};
    employeesCache.forEach(emp => { updatesByEmp[emp.id] = [...(employeeCatalogCache[emp.id] || [])]; });

    document.querySelectorAll('#stock-manage-list input[data-stock-emp]').forEach(input => {
        const empId = input.getAttribute('data-stock-emp');
        ...
        const assigned = updatesByEmp[empId] || (updatesByEmp[empId] = []);
        const idx = assigned.findIndex(a => a.productId === productId);
        if (idx === -1) {
            assigned.push({ productId, qty });
        } else {
            assigned[idx] = { ...assigned[idx], qty };
        }
    });

    const { db, doc, setDoc } = window.FB;
    const writes = Object.keys(updatesByEmp).map(empId =>
        setDoc(doc(db, 'config', 'employeeCatalog'), { [empId]: updatesByEmp[empId] }, { merge: true })
    );
...
// --- TRANSFER STOCK PRODUK (pindahkan stock dari satu kasir ke kasir lain) ---
// Tabel: baris = produk, kolom = tiap kasir terdaftar, isinya = stock kasir tsb untuk produk
// itu ("∞" kalau tidak diatur/tidak terbatas). Header tabel dibangun ulang tiap render karena
// jumlah kolom mengikuti jumlah karyawan yang terdaftar (bisa berubah-ubah).
...
// Pindahkan sejumlah stock dari satu kasir ke kasir lain untuk satu produk, pakai Firestore
// transaction supaya aman kalau ada 2 Admin transfer bersamaan. Validasi stock cukup dulu
// sebelum benar-benar dipindah — kalau tidak cukup, transfer dibatalkan & dikasih tahu jelas.
async function submitStockTransfer() {
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            if (fromQty == null) {
                // Kasir asal tidak terbatas (belum diatur stock khusus) -> tidak ada yang bisa "dipindah" secara angka pasti
...
//
// Stock sekarang MURNI milik kasir yang lagi transaksi (employeeId) — tidak ada lagi stock
// global gabungan. Yang dikurangi adalah qty di employeeCatalogCache[employeeId] (kalau produk
// itu memang punya jatah stock buat kasir ini; kalau kosong/tidak diatur berarti tidak terbatas,
// jadi tidak ada yang perlu dikurangi).
async function decreaseStockForOrder(items, employeeId) {
    if (!window.FB || !window.FB.ready || !employeeId) return; // tanpa kasir aktif, tidak ada stock kasir yang perlu dikurangi
    const { db, doc, runTransaction } = window.FB;
    const catalogRef = doc(db, 'config', 'employeeCatalog');
        ...
        await runTransaction(db, async (transaction) => {
            ...
            items.forEach(item => {
                if (item.variantSelections && item.variantSelections.length) {
                    // PAKET: kurangi stock dari tiap produk satuan yang dipilih di dalamnya
...
// --- METODE PEMBAYARAN (Admin nyalakan/matikan QRIS & Online/Shopee dari sidebar; Cash selalu
// aktif jadi tidak ada tombol toggle-nya) ---
...
// Sembunyikan tombol QRIS/SHOPEE di layar Checkout kalau dimatikan Admin, dan rapikan lebar
// grid tombol pembayaran mengikuti jumlah metode yang masih tampil (Cash selalu tampil).
function updateCheckoutPaymentButtons() {
    ...
    // Jaga-jaga: kalau metode yang lagi dipilih kasir ternyata baru saja dimatikan Admin
    // (real-time), balikin pilihan ke Cash biar tidak nyangkut di metode yang sudah tidak ada.
...
//   - qty null/undefined = aktif tapi TANPA batas stock khusus (dianggap tidak terbatas).
//   - qty angka = stock MILIK karyawan ini SENDIRI di produk ini (tidak ada stock global lagi).
//     Berkurang permanen tiap kali karyawan ini jual produknya, dan bisa dipindah antar
//     karyawan lewat "Transfer Stock Produk".
...
// Stock sekarang MURNI milik masing-masing kasir (tidak ada lagi stock global gabungan).
// Diatur Admin lewat "Kelola Stock" / "Katalog Per Karyawan", dan berkurang permanen tiap kali
// kasir itu jual produknya (lihat decreaseStockForOrder) — bisa juga dipindah antar kasir lewat
// "Transfer Stock Produk". Kosong/tidak diatur = dianggap tidak terbatas buat kasir tsb.
function getEffectiveStockForCurrentKasir(product) {
    const empId = getCurrentKasirEmployeeId();
    if (!empId) return null; // Admin / belum ada kasir aktif -> tidak ada batas stock ditampilkan
    const assigned = employeeCatalogCache[empId];
    const entry = assigned && assigned.find(a => a.productId === product.id);
    if (!entry || entry.qty == null) return null; // belum diatur / dikosongkan -> tidak terbatas
    return entry.qty;
}
...
// Kode absen sekarang DITURUNKAN OTOMATIS dari tanggal hari ini + nama toko, dihitung langsung
// di tiap HP (tidak disimpan/diambil dari server lagi) — jadi otomatis berganti sendiri setiap
// hari (tepat jam 00:00) TANPA Admin perlu tap "Generate" apa pun, dan selalu identik persis
// di semua perangkat pada tanggal yang sama (Admin & kasir tidak perlu saling sinkron manual).
// Kode manual (buat copy-paste) otomatis ikut berubah juga karena sumbernya sama persis.
...
async function renderAttendanceQR() {
    ...
    container.innerHTML = '';
    // Ukuran diperkecil (dulu 320x320, kelihatan kegedean di layar HP) — 180x180 masih gampang
    // di-scan dari jarak wajar tapi jauh lebih pas untuk layar sempit / tidak perlu discroll.
...
// --- REFRESH ABSEN (reset harian: hapus semua data penjualan & absen masuk/pulang kasir) ---
// Dipakai kalau Admin mau "mulai ulang dari nol" di hari yang sama (misal salah input massal) —
// menghapus SEMUA riwayat penjualan (koleksi 'sales') dan SEMUA rekam absen masuk/keluar
// (koleksi 'attendance'), jadi tiap kasir wajib absen masuk lagi sebelum bisa bertransaksi.
// Dibatasi 1x per hari lewat config/refreshAbsenLog (bukan cuma dicek di HP ini) supaya
// konsisten walau dipencet dari HP Admin yang berbeda.
function renderRefreshAbsenButton() {
    const btn = document.getElementById('refresh-absen-btn');
    const note = document.getElementById('refresh-absen-note');
    if (!btn) return;
    const alreadyDoneToday = refreshAbsenLastDate === getTodayDateStr();
    btn.disabled = alreadyDoneToday;
    btn.classList.toggle('opacity-50', alreadyDoneToday);
    btn.classList.toggle('cursor-not-allowed', alreadyDoneToday);
    if (note) {
        note.innerText = alreadyDoneToday
            ? 'Refresh Absen sudah dipakai hari ini. Bisa dipakai lagi besok.'
            : 'Menghapus semua data penjualan & absen masuk/pulang kasir hari ini — dipakai maksimal 1x per hari.';
    }
}

function openConfirmRefreshAbsen() {
    if (refreshAbsenLastDate === getTodayDateStr()) {
        showToast('Refresh Absen sudah dipakai hari ini. Coba lagi besok.', 'warn');
        return;
    }
    document.getElementById('modal-confirm-refresh-absen').classList.remove('hidden');
    document.getElementById('modal-confirm-refresh-absen').classList.add('flex');
}

function closeConfirmRefreshAbsen() {
    document.getElementById('modal-confirm-refresh-absen').classList.add('hidden');
    document.getElementById('modal-confirm-refresh-absen').classList.remove('flex');
}

async function confirmRefreshAbsenYes() {
    const todayStr = getTodayDateStr();
    if (!window.FB || !window.FB.ready) {
        showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
        return;
    }
    if (refreshAbsenLastDate === todayStr) {
        showToast('Refresh Absen sudah dipakai hari ini.', 'warn');
        closeConfirmRefreshAbsen();
        return;
    }
    closeConfirmRefreshAbsen();
    showToast('Memproses Refresh Absen...', 'info');

    const { db, doc, setDoc, deleteDoc } = window.FB;
    try {
        // Hapus semua riwayat penjualan hari ini (koleksi 'sales') — pakai firestoreId yang
        // sudah disimpan tiap doc lewat listener 'sales' (lihat initFirestoreSync).
        const salesDeletes = orderHistory
            .filter(o => o.firestoreId)
            .map(o => deleteDoc(doc(db, 'sales', o.firestoreId)));

        // Hapus semua rekam absen masuk/keluar (koleksi 'attendance') — docId dibangun ulang
        // dari `${tanggal}_${employeeId}`, sama seperti dipakai saveAttendanceRecord/autoCloseForgottenShifts.
        const attendanceDeletes = attendanceLogCache
            .filter(r => r.date && r.employeeId != null)
            .map(r => deleteDoc(doc(db, 'attendance', `${r.date}_${r.employeeId}`)));

        await Promise.all([...salesDeletes, ...attendanceDeletes]);

        // Catat tanggal refresh ini supaya tidak bisa dipakai lagi hari yang sama, di HP manapun.
        await setDoc(doc(db, 'config', 'refreshAbsenLog'), { lastDate: todayStr, lastAt: new Date().toISOString() });

        showToast('Refresh Absen berhasil! Semua kasir wajib absen masuk lagi.', 'success');
    } catch (err) {
        console.error('Gagal Refresh Absen:', err);
        showToast('Gagal melakukan Refresh Absen. Coba lagi.', 'warn');
    }
}
...
// Absen MASUK sekarang WAJIB untuk SEMUA kasir yang login PIN, setiap hari — tidak lagi
// bergantung pada apakah dia dijadwalkan di tabel Jadwal Kerja Karyawan (jadwal itu sekarang
// murni informasi jam mulai, bukan syarat wajib-tidaknya absen). Selama kasir belum absen
// masuk hari ini, dashboard tetap terkunci dan transaksi tidak bisa dimulai.
...
function saveAttendanceRecord(type, employee) {
    ...
    // OPTIMISTIC UPDATE: update cache lokal & buka gerbang SEKARANG JUGA, jangan nunggu listener
    // Firestore pulang-pergi ke server dulu. Sebelumnya dashboard cuma kebuka lewat snapshot
    // 'attendance' yang datang belakangan — kalau koneksi lagi lambat/putus-putus, kasir sudah
    // absen (dokumennya tersimpan) tapi dashboard-nya nyangkut kekunci sampai listener itu benar-benar
    // nyampe. Begitu snapshot asli datang nanti, cache ini ditimpa ulang dengan data yang sama, jadi aman.
...
// --- ABSEN KELUAR OTOMATIS (jaga-jaga kalau kasir lupa tap "Absen Keluar") ---
// Kalau ada rekam absen dari tanggal SEBELUM hari ini yang sudah absen masuk tapi belum absen
// keluar, sistem otomatis menutup shift itu sendiri (tercatat "keluar" jam 23:59 di tanggal
// tsb) begitu tanggal berganti / lewat jam 00:00 — jadi shift kasir yang lupa absen keluar
// tidak akan menggantung selamanya dan tidak mengunci hari berikutnya. Dipanggil tiap kali data
// absen dari server berubah, DAN tiap menit lewat setInterval (lihat init()) supaya tetap
// jalan walau tidak ada perubahan data lain persis di jam 00:00.
const _autoClosingShiftIds = new Set(); // cegah kirim setDoc dobel selagi menunggu balasan server
function autoCloseForgottenShifts() {
    if (!window.FB || !window.FB.ready) return;
    const todayStr = getTodayDateStr();
    const { db, doc, setDoc } = window.FB;
    attendanceLogCache.forEach((record) => {
        if (!record.date || record.date >= todayStr) return; // cuma tanggal yang sudah lewat
        if (!record.masukTime || record.keluarTime) return; // sudah lengkap / belum sempat masuk
        ...
        setDoc(doc(db, 'attendance', docId), {
            ...
            _autoClosingShiftIds.delete(docId); // biar dicoba lagi di sinkronisasi berikutnya
        });
    });
}
...
// --- PERBAIKAN ABSENSI (Admin bisa isi/perbaiki manual jam masuk & keluar karyawan yang lupa
// absen di tanggal tertentu — dibutuhkan supaya "Input Total Penjualan Hari Ini" di tanggal itu
// bisa dibuka, karena input tsb terkunci sampai kasirnya tercatat absen keluar) ---
...
// Ubah ISO timestamp jadi "HH:MM" versi JAM LOKAL HP ini (bukan UTC) — supaya konsisten dengan
// cara absen normal disimpan (`new Date().toISOString()` dari jam lokal perangkat).
...
// Kebalikannya: gabungkan tanggal + "HH:MM" yang diketik Admin (diasumsikan jam lokal HP ini,
// sama seperti jam dinding di toko) jadi ISO timestamp UTC yang benar buat disimpan ke Firestore.
...
// Tombol pintas: isi kolom Jam Keluar dengan 23:59, buat cepat "menutup" shift kemarin yang
// kelupaan absen keluar tanpa perlu tahu persis jam pulangnya.
...
function savePerbaikanAbsen() {
    ...
    if (keluarVal) {
        ...
        field.keluarAuto = false; // sudah dikonfirmasi manual oleh Admin, bukan lagi hasil auto-close sistem
    }
...
// --- NAVIGASI 2 LEVEL DI SIDEBAR ADMIN (menu utama -> grup) ---
// Level 1: daftar 6 tile (Profile Toko, Produk, QR Absen, Karyawan, Pembayaran, Laporan).
// Level 2: isi grup yang ditap, dengan tombol "Kembali" balik ke level 1. Tidak ada satupun
// menu yang dikunci/di-disable di sini — semua tile & isinya selalu bisa dipakai.
...
// Admin Panel sekarang tampil sebagai SIDEBAR yang geser dari kiri (menimpa halaman Kasir di
// baliknya, bukan lagi mengganti seluruh halaman) — dibuka lewat ikon gembok di header.
// `page-home`/`bottom-bar` SENGAJA dibiarkan tetap ada di baliknya (bukan disembunyikan) supaya
// terasa seperti drawer sungguhan; `inert` dipakai supaya tetap tidak bisa disentuh selagi
// sidebar terbuka walau kelihatan di balik backdrop gelap.
function showPage(page) {
    ...
    ['page-home', 'bottom-bar'].forEach((id) => {
        const el = document.getElementById(id);
        if (!el) return;
        // Jangan lepas `inert` di sini kalau memang sedang wajib terkunci karena gerbang absen/shift
        // (recomputeDashboardLock yang berwenang penuh soal itu) — di sini cuma menambah kunci
        // tambahan selagi sidebar admin terbuka, dan melepasnya lagi kalau memang sudah boleh.
        if (enteringAdmin) {
    ...
    if (enteringAdmin) {
        ...
        showAdminMenuLevel1(); // selalu mulai dari menu utama tiap kali sidebar dibuka
...
// Dipakai tombol X di sidebar & klik backdrop gelap di baliknya.
function closeAdminSidebar() {
    showPage('home');
}

// Logo/nama toko di tengah header — tap buat balik ke dashboard utama Kasir kapan saja
// (kalau lagi di Admin Panel, sidebar-nya ditutup dulu; kalau memang sedang di dashboard
// Kasir, tidak melakukan apa-apa).
