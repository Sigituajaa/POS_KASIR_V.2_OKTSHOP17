                ...
                ${employeesCache.map(emp => {
                    ...
                    <div class="min-w-0">
                        ...
                        <input
                            ...
                            value="${qty ?? ''}"
                            placeholder="∞"
                            onchange="updateEmployeeStock('${emp.id}', ${p.id}, this.value)"
...
// Simpan stock khusus satu kasir untuk satu produk. Kalau produk ini belum pernah diaktifkan
// buat kasir tsb (belum ada entry-nya di employeeCatalogCache), otomatis dibuatkan entry baru
// (artinya produk ini jadi aktif buat kasir tsb, sama seperti mencentang checkbox di
// "Katalog Per Karyawan").
function updateEmployeeStock(empId, productId, value) {
    if (!window.FB || !window.FB.ready) return showToast('Belum terhubung ke database. Coba lagi sebentar.', 'warn');
    const assigned = [...(employeeCatalogCache[empId] || [])];
    const idx = assigned.findIndex(a => a.productId === productId);
    const qty = (value === '' || value === null) ? null : Math.max(0, parseInt(value) || 0);
    if (idx === -1) {
    ...
    setDoc(doc(db, 'config', 'employeeCatalog'), { [empId]: assigned }, { merge: true }).then(() => {
        ...
        showToast(`Stock ${empName} diperbarui!`, 'success');
    }).catch((err) => {
        console.error('Gagal update stock per kasir:', err);
        showToast('Gagal menyimpan ke server.', 'warn');
    });
}
